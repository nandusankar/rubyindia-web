...
class Api::V1::UsersControllerTest < ActionController::TestCase
  ...
  include Api::V1::AuthenticationTestHelper
  fixtures :users

  def test_show_for_a_valid_user
    admin = users(:admin)
    params = auth_params(admin).merge(id: admin.email)
  ...
  def test_show_when_email_is_not_present
    ...
    params = auth_params(admin).merge(id: an_invalid_email)
    set_auth_headers!(admin)

    get :show, params

    assert_response 404
    assert_equal "User with email #{an_invalid_email} not found.", JSON.parse(response.body)['error']
  end
  ...
  def test_update_user_should_succeed_for_valid_data
    ...
    params = auth_params(admin).merge({ id: admin.email, user: { first_name: new_first_name } })
    set_auth_headers!(admin)
  ...
  def test_destroy_should_return_error_if_email_is_not_present_in_database
    ...
    params = auth_params(admin).merge(id: email)
    ...
    assert_response 404
    assert_equal "User with email #{email} not found.", JSON.parse(response.body)['error']
  end
